...
// CONFIGURAÇÕES
const CACHE_NAME = 'agenda-cache-v1.0.0';
const OFFLINE_CACHE = 'agenda-offline-v1';
const API_CACHE = 'agenda-api-v1';
...
self.addEventListener('install', (event) => {
  console.log('[SW] Instalando Service Worker versão 1.0.0');
...
async function cacheFirst(request) {
  ...
  try {
    ...
    if (request.headers.get('Accept').includes('text/html')) {
      ...
      const offlineResponse = await offlineCache.match('/agenda/offline.html');
      if (offlineResponse) {
...
async function checkForUpdates() {
  try {
    const response = await fetch('/agenda/version.json', {
      cache: 'no-store'
    });
